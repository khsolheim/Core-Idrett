...
class _PendingFineCard extends ConsumerWidget {
  ...
  void _approveFine(BuildContext context, WidgetRef ref) async {
    ...
    if (result != null && context.mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Bøte godkjent')),
      );
    }
  }
  ...
  void _rejectFine(BuildContext context, WidgetRef ref) async {
    ...
    if (result != null && context.mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Bøte avvist')),
      );
    }
  }
}
...
class _PendingAppealCard extends ConsumerWidget {
  ...
  void _acceptAppeal(BuildContext context, WidgetRef ref) async {
    ...
    if (result != null && context.mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Klage godtatt - bøte fjernet')),
      );
    }
  }

  void _showRejectAppealDialog(BuildContext context, WidgetRef ref) {
    final extraFeeController = TextEditingController();

    showDialog(
      context: context,
      builder: (context) => AlertDialog(
        title: const Text('Avslå klage'),
        content: Column(
          mainAxisSize: MainAxisSize.min,
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            const Text('Vil du legge til ekstragebyr for å klage uten grunn?'),
            const SizedBox(height: 16),
            TextField(
        ...
        actions: [
          ...
          TextButton(
            onPressed: () async {
              ...
              await ref.read(appealNotifierProvider.notifier).resolveAppeal(
                    appealId: appeal.id,
                    accepted: false,
                    teamId: teamId,
                    extraFee: extraFee,
              ...
              if (context.mounted) {
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(
                    content: Text(extraFee != null
                        ? 'Klage avslått med ${extraFee.toStringAsFixed(0)} kr ekstragebyr'
                        : 'Klage avslått'),
                  ),
                );
              }
            },
            child: const Text('Avslå', style: TextStyle(color: Colors.red)),
          ),
        ],
      ),
    );
